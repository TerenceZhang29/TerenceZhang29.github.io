...
7. After the user approves the required changes, hand them off to `planner-agent` for an implementation plan.
...
### Planning Handoff
When the required changes are approved, provide the root cause, ordered change list, affected files and symbols, regression tests, validation commands, acceptance criteria, and known risks for `planner-agent`. Do not claim that a plan or implementation exists until the receiving agent produces it.
